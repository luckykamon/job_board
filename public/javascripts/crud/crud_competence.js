...
function boutton_modifier(id) {
    let dico = {};
    dico.compe_nom = $("#" + id).children()[0].textContent.trim();
    $("#" + id).children().remove();
...
function affichage_ligne_modifier(id, dico, dico_select = false) {
    ...
    afficher += `
    <td>
        <input class="form-control" type="text" id="compe_nom_input_${id}" value="${dico.compe_nom}">
      </td>
      <td>
...
function boutton_envoyer(id) {
    let dico = {};
    dico.compe_id = id;
    dico.compe_nom = $("#compe_nom_input_" + id)[0].value;
    if (dico.compe_nom == "") {
        alert("Veuillez ajouter un compe_nom")
    } else {
...
function ajouter_ligne_ById(id, element) {
    ...
    let afficher = "";
    afficher += `
    <td>${element.compe_nom}</th>
    <td>
      <button type="button" class="btn btn-warning" onclick="boutton_modifier('${element.compe_id}')">Modifier</button> <button type="button" class="btn btn-danger" onclick="boutton_supprimer('${id}')">Supprimer</button>
    </td>
    `
...
function callAPI_remove(id) {
    let envoyer;
    envoyer = JSON.stringify({ compe_id: id });
...
(function (window, document) {
    ...
    function boutton_ajouter() {
        let dico = {};
        dico.compe_nom = $("#compe_nom_input")[0].value;
        if (dico.compe_nom == "") {
            alert("Veuillez ajouter un compe_nom")
        } else {
            callAPI_ajout(dico, "compe");
        }
    }
    ...
    */
    function API_liste_information() {
        $.ajax({
            url: "/compe", // METHOD GET par defaut
            contentType: "application/json", // Pour préciser le type de données qui transite
            dataType: 'json',
            success: function (result) {
    ...
    function affichage_liste_informations(liste_informations) {
        liste_informations.forEach(element =>
            ...
            <tbody class="liste_information">
              <tr id="${element.compe_id}">
                <td>${element.compe_nom}</th>
                <td>
                  <button type="button" class="btn btn-warning" onclick="boutton_modifier('${element.compe_id}')">Modifier</button> <button type="button" class="btn btn-danger" onclick="boutton_supprimer('${element.compe_id}')">Supprimer</button>
                </td>
              </tr>
            </tbody>`)
        );
